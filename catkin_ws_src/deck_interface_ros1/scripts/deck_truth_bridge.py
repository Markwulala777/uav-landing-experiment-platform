...
class DeckTruthBridge:
    def __init__(self):
        ...
        self.wamv_model = rospy.get_param("~wamv_model", "wamv")
        self.uav_model = rospy.get_param("~uav_model", "iris_0")
        self.deck_offset = rospy.get_param("~deck_offset_xyz", [0.0, 0.0, 1.25])
        self.target_offset = rospy.get_param("~landing_target_offset_xyz", [0.5, 0.0, 1.25])
        ...
        rospy.loginfo("deck_interface_ros1 started. Waiting for /gazebo/model_states.")
    ...
    def model_states_cb(self, msg):
        try:
            wamv_idx = msg.name.index(self.wamv_model)
            uav_idx = msg.name.index(self.uav_model)
        except ValueError:
            rospy.logwarn_throttle(5.0, "deck_interface_ros1 could not find models '%s' and '%s'.", self.wamv_model, self.uav_model)
            return

        wamv_pose = msg.pose[wamv_idx]
        wamv_twist = msg.twist[wamv_idx]
        uav_pose = msg.pose[uav_idx]
        uav_twist = msg.twist[uav_idx]

        stamp = rospy.Time.now()

        deck_position, deck_quaternion = self.apply_offset(wamv_pose, self.deck_offset)
        target_position, target_quaternion = self.apply_offset(wamv_pose, self.target_offset)
        ...
        relative_linear_world = [
            uav_twist.linear.x - wamv_twist.linear.x,
            uav_twist.linear.y - wamv_twist.linear.y,
            uav_twist.linear.z - wamv_twist.linear.z,
        ]
        relative_angular_world = [
            uav_twist.angular.x - wamv_twist.angular.x,
            uav_twist.angular.y - wamv_twist.angular.y,
            uav_twist.angular.z - wamv_twist.angular.z,
        ]
        ...
        self.deck_twist_pub.publish(
            self.build_twist(
                stamp,
                self.world_frame,
                [wamv_twist.linear.x, wamv_twist.linear.y, wamv_twist.linear.z],
                [wamv_twist.angular.x, wamv_twist.angular.y, wamv_twist.angular.z],
            )
        )
...
def main():
    rospy.init_node("deck_truth_bridge")
    DeckTruthBridge()
    rospy.spin()
