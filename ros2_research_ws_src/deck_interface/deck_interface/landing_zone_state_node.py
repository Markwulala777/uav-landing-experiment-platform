...
from uav_usv_landing_msgs.msg import LandingZoneState
...
class LandingZoneStateNode(Node):
    def __init__(self):
        super().__init__("landing_zone_state")
        ...
        self.zone_state_pub = self.create_publisher(LandingZoneState, "/deck/landing_zone_state", 10)
        self.target_pose_debug_pub = self.create_publisher(
            PoseStamped, "/deck_interface/truth/landing_target_pose", 10
        )
        self.target_twist_debug_pub = self.create_publisher(
            TwistStamped, "/deck_interface/truth/landing_target_twist", 10
        )
        ...
        self.get_logger().info("deck_interface landing-zone state publisher is running.")
